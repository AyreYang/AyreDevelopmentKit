    ...
    public abstract class TableEntity : IDisposable
    {
        ...
        internal List<DBColumn> PrimaryList
        {
            get
            {
                ...
                return columns.FindAll(col => col.KeyType == KeyType.Primary);
            }
        }
        ...
        private Dictionary<string, DBForeign> Foreigns { get; set; }
        ...
        internal List<DbCommand> GetUpdateCommands(DatabaseCore accessor = null)
        {
            var commands = new List<DbCommand>();
            var dba = (accessor != null) ? accessor : this.accessor;
            if (dba == null) return commands;
        ...
        internal List<DbCommand> GetSaveCommands(DatabaseCore accessor = null)
        {
            ...
            if (this.EntityStatus > EntityState.RAW)
            {
                ...
                if (this.EntityStatus == EntityState.ASSIGNED) list = GetInsertCommands(accessor);
                if (this.EntityStatus == EntityState.CHANGED) list = GetUpdateCommands(accessor);
        ...
        {
            if (accessor == null) return 0;
            var lst_commands = GetSaveCommands();
            var result = accessor.ExecuteSQLCommand(lst_commands);
            if (result >= 0) result = Fresh();
            return result;
        }
        public long Delete()
        {
            ...
            var result = accessor.ExecuteSQLCommand(lst_commands);
            if (result > 0) this.EntityStatus = EntityState.DELETED;
            return result;
        }
        ...
        public virtual bool CreateTable(string script)
        {
            if (accessor == null) return false;
            if (string.IsNullOrWhiteSpace(script)) return false;
            if (accessor.TableExists(this.TableName)) return true;

            var ret = accessor.ExecuteSQLCommand(accessor.CreateCommand(script, null));
            return ret >= 0;
        }
